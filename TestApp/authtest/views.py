        ...
        if form.is_valid():
            ...
            if user_qs.exists():
                ...
                if not user.is_active:
                    ...
                    form.add_error(
                        "email", "Пользователь с таким email уже существует и активен."
                    )
            else:
                ...
                Profile.objects.create(
                    user=user, patronymic=form.cleaned_data["patronymic"]
                )
                UserRole.objects.get_or_create(user=user, role=user_role)
                return redirect("login")
    else:
        ...
        try:
            ...
            if admin_role:
                has_admin = UserRole.objects.filter(
                    user=request.user, role=admin_role
                ).exists()
...
# Можно использовать декоратор для проверки аутентификации и в случае неаутентифицированного пользователя, будет редирект на страницу логина
# @login_required
